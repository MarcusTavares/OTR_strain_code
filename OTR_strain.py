...
# Input parameters
# ---------------------------------------------------------------------------------------------------------------------------------------
COD_exp = [22.8531E-3, 34.9888E-3, 46.8041E-3, 47.8886E-3, 51.4634E-3, 57.6158E-3, 64.3079E-3, 68.1088E-3, 72.9887E-3, 75.2334E-3,
           79.4197E-3, 82.3293E-3, 86.6720E-3, 88.5974E-3, 93.5763E-3]   # Crack-opening displacement [µm]
L_exp = [94.9200, 10.1700, 5.8512, 4.4035, 2.8926, 2.3952, 2.2442,2.1464,
         1.9867, 1.8956, 1.7578, 1.6995, 1.6345, 1.5859, 1.5093]         # Crack spacing [µm]
h_s= 120                                                                 # Thickness of the substrate [µm]
h_c = 20E-3                                                              # Thickness of the coating [µm]
OTR_s = 35                                                               # Oxygen transmission rate of the substrate [cm^3/m^2/day]
OTR_c = 0.0001                                                           # Oxygen transmission rate of the coating [cm^3/m^2/day]
D_s = 1                                                                  # Diffusion coefficient for oxygen in the substrate [µm^2/s]
F = 1                                                                    # The constant flux at the crack [mol/µm^4]

# ---------------------------------------------------------------------------------------------------------------------------------------
# code starts (strain and strain range must be adjusted from the fragmentation tests)
# ---------------------------------------------------------------------------------------------------------------------------------------
...
while count < strain_range:
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------
    # Define Dirichlet boundary conditions on top and bottom boundaries
    # -----------------------------------------------------------------------------------------------------------------------------------
    bcs = [DirichletBC(V, 0.0, boundaries, 4)]

    # -----------------------------------------------------------------------------------------------------------------------------------
    # Define measures associated with the interior domains and exterior boundaries
    # -----------------------------------------------------------------------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------
    Functional = (inner(D_s*grad(u), grad(v))*dx(0) + inner(D_s*grad(u), grad(v))*dx(1)
        - g_L*v*ds(1) - g_R*v*ds(3) - g_coating*v*ds(2) - g_crack*v*ds(5)
        - f*v*dx(0) - f*v*dx(1))

    # -----------------------------------------------------------------------------------------------------------------------------------
    # Separate left and right hand sides of the functional
    # -----------------------------------------------------------------------------------------------------------------------------------
    a, L = lhs(Functional), rhs(Functional)
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------
    # Extract degrees of freedom (DoF) indices
    # -----------------------------------------------------------------------------------------------------------------------------------
    x = dof_x[:, 0]
...
# ---------------------------------------------------------------------------------------------------------------------------------------
C_FEM = np.divide((np.multiply((np.divide(COD_exp,L_exp)),(h_s*F))),max_c)

# ---------------------------------------------------------------------------------------------------------------------------------------
# Calculate oxygen transmission rate when coating is crack free
# ---------------------------------------------------------------------------------------------------------------------------------------
h_cp = h_s + h_c
OTR_ILT =np.power(((h_s/(h_cp*OTR_s)) + (h_c/(h_cp*OTR_c))),-1)

# ---------------------------------------------------------------------------------------------------------------------------------------
# OTR-strain prediction
# ---------------------------------------------------------------------------------------------------------------------------------------
